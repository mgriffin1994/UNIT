...
def get_model_list(dirname, key):
  if os.path.exists(dirname) is False:
    return None
  fake_key = "kl_" + key #need this for kl_dis and dis (so kl_kl is always invalid no prob, kl_dis is invalid when key is dis)
  gen_models = [os.path.join(dirname, f) for f in os.listdir(dirname) if
                os.path.isfile(os.path.join(dirname, f)) and key in f and not (fake_key in f) and "pkl" in f]
...
# Get model list for load
def get_model_version(dirname, key, version):
  if os.path.exists(dirname) is False:
    return None
  fake_key = "kl_" + key #need this for kl_dis and dis (so kl_kl is always invalid no prob, kl_dis is invalid when key is dis)
  gen_models = [os.path.join(dirname, f) for f in os.listdir(dirname) if
  ...
  if gen_models is None:
    return None
  gen_models.sort()
  last_model_name = gen_models[version]
  return last_model_name
